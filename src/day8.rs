...
impl VM {
    pub fn parse(input: String) -> Self {
        Self {
            instructions: input.lines().map(|line| (Instruction::parse(line.to_owned()), false)).collect(),
            acc: 0, 
            ptr: 0
        }
    }
...
pub fn solve_part2(inp: &String) -> i32 {
    ...
    let flipping_targets: Vec<_> = vm.instructions.iter().enumerate().filter(|(_,(instruction,_))| {
